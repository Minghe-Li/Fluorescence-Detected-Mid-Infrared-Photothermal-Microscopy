function [result,data] = acquireData(boardHandle, AcqParameters)
...
preTriggerSamples = AcqParameters.PreTriggerSamples;

% TODO: Select the number of post-trigger samples per record
postTriggerSamples = AcqParameters.PostTriggerSamples;

% TODO: Specify the number of records per channel per DMA buffer
recordsPerBuffer = AcqParameters.RecordsPerBuffer;

% TODO: Specifiy the total number of buffers to capture
buffersPerAcquisition = AcqParameters.BuffersPerAcquisition;

% TODO: Select which channels to capture (A, B, or both)
channelMask = 1+2; %+ 2; %CHANNEL_A+CHANNEL_B
...
if retCode ~= 512 %ApiSuccess
    fprintf('Error: AlazarSetRecordSize failed -- %s\n', errorToText(retCode));
    return
end

% TODO: Select AutoDMA flags as required
admaFlags = 1 + 512; %ADMA_EXTERNAL_STARTCAPTURE + ADMA_TRADITIONAL_MODE;
...
while ~captureDone  
    ...
    if AcqParameters.Timeline 
        [retCode, boardHandle, bufferOut] = ...
            AlazarWaitAsyncBufferComplete(boardHandle, pbuffer, (AcqParameters.Timeline+1)*1000);
    else
        [retCode, boardHandle, bufferOut] = ...
            AlazarWaitAsyncBufferComplete(boardHandle, pbuffer, 30000);
    end
    if retCode == 512
        % This buffer is full
        bufferFull = true;
        captureDone = false;
    elseif retCode == 579 %ApiWaitTimeout
