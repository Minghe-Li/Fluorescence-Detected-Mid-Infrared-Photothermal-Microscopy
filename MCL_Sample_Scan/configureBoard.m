function [result] = configureBoard(boardHandle,ConfigParameters)
...
samplesPerSec = ConfigParameters.GlobalSamplingRate;

retCode = ...
    AlazarSetCaptureClock(  ...
        boardHandle,        ... % HANDLE -- board handle
        INTERNAL_CLOCK,     ... % U32 -- clock source id   EXTERNAL_CLOCK_10MHz_REF
        ConfigParameters.SamplingRate, ... % U32 -- sample rate id   160e6 
        CLOCK_EDGE_RISING,  ... % U32 -- clock edge id
        0                   ... % U32 -- clock decimation   8000
        );
...
retCode = ...
    AlazarInputControl(     ...
        ...
        ConfigParameters.ChanARange, ... % U32 -- input range id
        ConfigParameters.ChanAImp    ... % U32 -- input impedance id
        );
if retCode ~= ApiSuccess
    fprintf('Error: AlazarInputControl failed -- %s\n', errorToText(retCode));
    return
end
if ConfigParameters.CardSupportBW
...
retCode = ...
    AlazarInputControl(     ...
        ...
        DC_COUPLING,        ... % U32 -- input coupling id
        ConfigParameters.ChanBRange, ... % U32 -- input range id
        ConfigParameters.ChanBImp    ... % U32 -- input impedance id
        );
if retCode ~= ApiSuccess
    fprintf('Error: AlazarInputControl failed -- %s\n', errorToText(retCode));
    return
...
if ~ConfigParameters.External_Trigger
    retCode = ...
        AlazarSetTriggerOperation( ...
            ...
            ConfigParameters.TriggerLevel,                ... % U32 -- trigger level from 0 (-range) to 255 (+range)
...
retCode = ...
    AlazarSetExternalTrigger( ...
        ...
        ETR_1V              ... % U32 -- external trigger range id
        );
if retCode ~= ApiSuccess
